...
var CameraController = class extends TrackballControls {
	constructor(e, t, n = {}) {
		...
		e.near = -this.minDistance, e.far = this.maxDistance, e.updateProjectionMatrix(), this._shiftDown = !1, this._storedRotateSpeed = this.rotateSpeed, window.addEventListener("keydown", (e) => {
			e.key === "Shift" && !this._shiftDown && (this._shiftDown = !0, this._storedRotateSpeed = this.rotateSpeed, this.rotateSpeed = 0);
		}), window.addEventListener("keyup", (e) => {
			e.key === "Shift" && this._shiftDown && (this._shiftDown = !1, this.rotateSpeed = this._storedRotateSpeed);
		}), this.setParams(n), this._initDefaultViews();
	}
	onChange(e) {
	...
	_updateShiftState() {
		let e = window.event ? window.event.shiftKey : !1;
...
function toVector3(e, t = "value") {
	if (console.log("toVector3 called with value:", e), e instanceof THREE.Vector3) return e;
	if (Array.isArray(e) && e.length === 3) return new THREE.Vector3(e[0], e[1], e[2]);
	if (e && typeof e == "object" && "length" in e && e.length === 3) return new THREE.Vector3(...e);
	if (e && typeof e == "object" && "x" in e && "y" in e && "z" in e) return new THREE.Vector3(e.x, e.y, e.z);
...
function clearObjects(e, t = null) {
	[...e.children].forEach((n) => {
		t !== null && (!n.userData || n.userData.uuid !== t) || (n instanceof THREE.Group ? clearGroup(e, n) : !(n instanceof THREE.Camera) && !(n instanceof THREE.Light) && clearObject(e, n));
	});
}
function clearGroup(e, t) {
	t.children.forEach((t) => {
		t instanceof THREE.Group ? clearGroup(e, t) : clearObject(e, t);
	});
}
...
function calculateQuaternion(e, t) {
	let n = new THREE.Matrix4().lookAt(e, t, new THREE.Vector3(0, 1, 0)), r = new THREE.Quaternion().setFromRotationMatrix(n), i = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), Math.PI / 2);
...
var WeasScene = class extends THREE.Scene {
	constructor(e) {
	...
	getProjectedBoundingBox(e = [
		...
		], r = new THREE.Matrix4().lookAt(e, new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 1, 0)), i = new THREE.Vector3(Infinity, Infinity, Infinity), a = new THREE.Vector3(-Infinity, -Infinity, -Infinity);
		n.forEach((e) => {
...
var LegendHUD = class {
	constructor(e, t = {}) {
		this.hud = e, this.paramsSchema = {
			...
			position: "bottom-right",
			panelKey: "legend"
		}, t), this.entries = /* @__PURE__ */ new Map(), this.container = document.createElement("div"), this._applyContainerStyle(), this.settings.heading && (this.headingEl = document.createElement("div"), this.headingEl.textContent = this.settings.heading, this.headingEl.style.fontSize = `${this.settings.headingFontSize}px`, this.headingEl.style.marginBottom = `${this.settings.rowGap}px`, this.container.appendChild(this.headingEl)), this.hud.addHTMLPanel(this.config.panelKey, this.container, {
	...
	_createShape(e, t, n) {
		...
		switch (applyStyle(r, LEGEND_SHAPE_BASE_STYLE), r.style.width = `${n}px`, r.style.height = `${n}px`, e) {
			case "sphere":
				r.style.borderRadius = "50%", r.style.background = `radial-gradient(circle at 30% 30%, #ffffffaa, ${t} 65%, #00000044)`;
				break;
			case "cube":
	...
	updateSettings(e) {
		...
		}), this.headingEl && (this.headingEl.style.fontSize = `${this.settings.headingFontSize}px`, "heading" in e && (this.headingEl.textContent = this.settings.heading));
	}
	getSchema() {
	...
	addButton(e, { label: t = "", hint: n = "", onClick: r } = {}) {
		...
		}), r && i.addEventListener("click", r), i.addEventListener("pointerdown", (e) => e.stopPropagation()), i.addEventListener("pointerup", (e) => e.stopPropagation()), this.container.appendChild(i), this.buttons.set(e, i);
	}
	removeButton(e) {
	...
	initCoordScene() {
		...
		let t = new THREE.DirectionalLight(16777215, 2);
		t.position.set(10, 10, 10), e.add(t);
		let n = new THREE.AxesHelper(.5), r = new THREE.Group();
		r.add(n), e.add(r);
		let i = new THREE.OrthographicCamera(-2.5, 2.5, 2.5, -2.5, .1, 100);
	...
	setHTMLPanelVisible(e, t) {
		let n = this.htmlElements.get(e);
		n && (n.visible = t, n.element.style.display = t ? "flex" : "none");
	}
	render(e) {
		this.miniScenes.forEach(({ scene: t, camera: n, canvas: r, rotation: i, visible: a }) => {
			...
			let o = r.getBoundingClientRect(), s = this.container.getBoundingClientRect(), c = o.left - s.left, l = s.height - (o.top - s.top) - r.height, u = this.renderer, d = new THREE.Vector4();
	...
	constructor(e, t, n) {
		this.name = e, this.geometry = t, this.material = n, this.object3D = new THREE.Mesh(t, n);
	}
	...
	constructor(e, t) {
		this.containerElement = e, this.tjsConfig = t.tjsConfig || defaultTjsConfig, this.weas = t, this.scene = new WeasScene(this), this.objects = {}, this.materials = {}, this.meshes = {}, this.lights = {}, this.renderers = {}, this._cameraType = "Orthographic", this.sceneView = {
			left: 0,
			bottom: 0,
			width: 1,
			height: 1
	...
	set cameraType(e) {
		this._cameraType = e, this.cameraController = new CameraController(this.camera, this.renderers.MainRenderer.renderer.domElement);
	}
	get camera() {
		if (this._cameraType === "Orthographic") return this.orthographicCamera;
	}
	init() {
		this.scene.background = new THREE.Color(16777215);
		let e = this?.tjsConfig?.renderConfig || defaultTjsConfig.renderConfig, t = new THREE.WebGLRenderer(e);
		t.autoClear = !1;
		...
		this.orthographicCamera = new OrthographicCamera(-l, l, 10, -10, 1, 2e3, this), this.orthographicCamera.layers.enable(1), this.camera.position.set(0, -100, 0), this.camera.lookAt(0, 0, 0), this.scene.add(this.camera);
		let u = new THREE.DirectionalLight(16777215, 2);
		u.position.set(50, 50, 100), this.addLight("MainLight", u), this.camera.add(u);
		let d = new THREE.AmbientLight(4210752, 20);
		this.addLight("AmbientLight", d), this.cameraController = new CameraController(this.camera, t.domElement), this.cameraController.onChange(() => {
			this.render();
	...
	updateCameraAndControls({ lookAt: e = null, direction: t = [
		0,
		0,
		1
	], zoom: n = 1, fov: r = 50, padding: i = 10 }) {
	...
	renderSceneInfo(e, t, n, r, i, a, o) {
		let s = o.getSize(new THREE.Vector2());
	...
	render() {
		this.cameraController.update(), this.renderers.MainRenderer.renderer.clear(), this.weas?.textManager?.updateLabelSizes?.(this.camera, this.renderers.MainRenderer.renderer), this.weas?.avr?.ALManager?.updateLabelSizes?.(this.camera, this.renderers.MainRenderer.renderer), this.weas?.avr?.highlightManager?.updateLabelSizes?.(this.camera, this.renderers.MainRenderer.renderer), this.renderers.LabelRenderer.renderer.render(this.scene, this.camera), this.renderSceneInfo(this.scene, this.camera, this.sceneView.left, this.sceneView.bottom, this.sceneView.width, this.sceneView.height, this.renderers.MainRenderer.renderer), this.hud && this.hud.render(this.camera), this.cameraController.update();
	}
	exportImage(e = 2) {
		e = Math.min(e, 3);
	...
	async exportAnimation({ format: e = "webm", fps: t = 12, startFrame: n = 0, endFrame: r = null, mimeType: i = null, resolution: a = 2, frameCount: o = null, setFrame: s = null, getFrame: c = null, isPlaying: l = null, pause: u = null, play: d = null } = {}) {
		...
		let f = this.renderers.MainRenderer.renderer, p = f.getPixelRatio(), m = Math.min(a, 3), h = f.getSize(new THREE.Vector2());
...
var GUIManager = class {
	...
	addMaterialsFolder() {
		let e = this.gui.addFolder("Materials"), t = this.weas.materialsRegistry, n = () => {
		...
		this.refreshMaterials = () => {
			n(), this.refreshShapes && this.refreshShapes();
		}, t.onChange && t.onChange(this.refreshMaterials), this.refreshMaterials();
	}
	addShapeOperationsFolder() {
		let e = this.gui.addFolder("Shapes"), t = this.weas.shapeRegistry;
	...
	addCameraControlsFolder() {
		let e = this.gui.addFolder("Camera Views"), t = this.weas.tjs.cameraController, n = () => {
			for (; e.__controllers.length > 0;) e.remove(e.__controllers[0]);
			t.list().forEach((n) => {
	...
	addCameraSettingsFolder() {
		let e = this.gui.addFolder("Camera Settings"), t = this.weas.tjs.cameraController;
	...
	addHUDSettingsFolder() {
		let e = this.gui.addFolder("HUD Settings"), t = this.weas.tjs.hud;
		if (!t) return;
		let n = () => {
	...
	addLegendHUDFolder() {
		let e = this.gui.addFolder("Legend Appearance"), t = this.weas.tjs.hud.legendHUD;
	...
	addFolderFromSchema(e, t, n, r) {
		for (let [i, a] of Object.entries(n)) {
			...
			switch (a.type) {
				case "number":
					n = e.add(t, i, a.min, a.max, a.step);
					break;
				case "color":
					n = e.addColor(t, i);
					break;
				...
				case "boolean":
					...
					break;
				case "select":
					a.options && (n = e.add(t, i, a.options));
					break;
				default: continue;
			}
			let o = a.label ?? i;
			typeof r == "function" ? n?.name(o).onChange(() => {
				r(i, t[i]);
			}) : n?.name(o);
		}
	}
	...
	setupGUI(e) {
		...
		Object.keys(r).forEach((e) => {
			...
			let r = addController(e, i, t, n, resolveOptions(n.options, this));
...
function addController(e, t, n, r, i) {
	return r.type === "color" ? e.addColor(t, n) : r.type === "boolean" ? e.add(t, n) : r.type === "number" && r.min !== void 0 && r.max !== void 0 ? e.add(t, n, r.min, r.max) : r.type === "select" && i || i ? e.add(t, n, i) : e.add(t, n);
}
...
var transform_exports = /* @__PURE__ */ __exportAll({
	...
	constructor({ weas: e, vector: t = new THREE.Vector3(), constraintType: n = null, axis: r = null, planeNormal: i = null }) {
		super(e), this.currentFrame = e.avr.currentFrame, this.selectedAtomsIndices = Array.from(this.stateGet("viewer.selectedAtomsIndices", []) || []), this.selectedObjects = e.selectionManager.selectedObjects, Array.isArray(t) && (t = new THREE.Vector3(t[0], t[1], t[2])), this.vector = t.clone(), this.constraintType = n || null, this.axis = r ? r.clone() : new THREE.Vector3(), this.normal = new THREE.Vector3(), this.planeNormal = new THREE.Vector3(), this.planeU = new THREE.Vector3(), this.planeV = new THREE.Vector3(), this.distance = 0, this.uDistance = 0, this.vDistance = 0, this.constraintType === "axis" && this.axis.lengthSq() === 0 && this.axis.set(1, 0, 0), this.constraintType === "normal" && i && this.normal.copy(i), this.constraintType === "plane" && i && this.planeNormal.copy(i), this.refreshConstraintStateFromVector(), this.uiFields = this.buildUISchema();
	}
	execute() {
	...
	ensurePlaneBasis() {
		...
		let e = this.planeNormal, t = Math.abs(e.x) < .9 ? new THREE.Vector3(1, 0, 0) : new THREE.Vector3(0, 1, 0), n = new THREE.Vector3().crossVectors(e, t);
		n.lengthSq() === 0 && n.crossVectors(e, new THREE.Vector3(0, 0, 1)), n.normalize();
		let r = new THREE.Vector3().crossVectors(e, n).normalize();
	...
	constructor({ weas: e, axis: t, angle: n, centroid: r = null }) {
		super(e), this.currentFrame = e.avr.currentFrame, this.selectedAtomsIndices = Array.from(this.stateGet("viewer.selectedAtomsIndices", []) || []), this.selectedObjects = e.selectionManager.selectedObjects, Array.isArray(t) && (t = new THREE.Vector3(t[0], t[1], t[2])), this.axis = t, this.angle = n, this.centroid = r;
	}
	execute() {
	...
	undo() {
		...
		let e = new THREE.Vector3(1 / this.scale.x, 1 / this.scale.y, 1 / this.scale.z);
	...
	getCentroidNDC(e = null) {
		if (this.weas.avr.selectedAtomsIndices.length > 0) e || (e = new THREE.Vector3(0, 0, 0), this.weas.avr.selectedAtomsIndices.forEach((t) => {
			e.add(new THREE.Vector3(...this.weas.avr.atoms.positions[t]));
		}), e.divideScalar(this.weas.avr.selectedAtomsIndices.length));
		else if (this.weas.selectionManager.selectedObjects.length > 0) e || (e = new THREE.Vector3(0, 0, 0), this.weas.selectionManager.selectedObjects.forEach((t) => {
			e.add(t.position);
	...
	updateRotationReference() {
		...
		if (e.length === 3) {
			...
			if (t && t[e[0]] && t[e[1]] && t[e[2]]) {
				let n = new THREE.Vector3(...t[e[0]]), r = new THREE.Vector3(...t[e[1]]), i = new THREE.Vector3(...t[e[2]]), a = r.clone().sub(n).cross(i.clone().sub(n));
				if (a.lengthSq() > 0) {
					let e = a.normalize();
			...
			let t = new THREE.Vector3(...this.weas.avr.atoms.positions[e[0]]), n = new THREE.Vector3(...this.weas.avr.atoms.positions[e[1]]), r = n.clone().sub(t);
			if (r.lengthSq() > 0) {
		...
		if (t.length === 3) {
			...
			if (e && e[t[0]] && e[t[1]] && e[t[2]]) {
				let n = new THREE.Vector3(...e[t[0]]), r = new THREE.Vector3(...e[t[1]]), i = new THREE.Vector3(...e[t[2]]), a = r.clone().sub(n).cross(i.clone().sub(n));
				if (a.lengthSq() > 0) {
					let e = a.normalize();
					e.dot(this.cameraDirection) < 0 && e.negate(), this.rotationAxis.copy(e), this.rotationCentroid.copy(n.clone().add(r).add(i).multiplyScalar(1 / 3));
					return;
		...
		t.length > 0 ? (t.forEach((e) => {
			this.rotationCentroid.add(new THREE.Vector3(...this.weas.avr.atoms.positions[e]));
	...
	storeInitialObjectState() {
		this.weas.avr.selectedAtomsIndices.forEach((e) => {
			...
			t.decompose(n, new THREE.Quaternion(), new THREE.Vector3()), this.initialAtomPositions.set(e, n.clone());
	...
	getTranslateVector(e, t) {
		...
		if (this.translatePlanePending) return new THREE.Vector3(0, 0, 0);
	...
	setTranslateAxisFromAtoms() {
		...
		let n = new THREE.Vector3(...t[e[0]]), r = new THREE.Vector3(...t[e[1]]).clone().sub(n);
	...
	getScaleVector(e, t) {
		...
		let i = new THREE.Vector2().subVectors(n, this.centroidNDC), a = new THREE.Vector2().subVectors(r, this.centroidNDC).length() / i.length();
		return new THREE.Vector3(a, a, a);
	}
	getRotationAngle(e, t) {
		let n = this.getNDC(t), r = this.getNDC(e);
		if (n.equals(r)) return;
		let i = new THREE.Vector2().subVectors(n, this.centroidNDC), a = new THREE.Vector2().subVectors(r, this.centroidNDC);
		i.normalize(), a.normalize();
		let o = Math.acos(i.dot(a));
		return i.x * a.y - i.y * a.x < 0 && (o = -o), o = THREE.MathUtils.radToDeg(o), o;
	}
};
//#endregion
//#region src/core/EventHandlers.js
function matchKey$1(e, t) {
	let n = t[t.length - 1], r = t.slice(0, -1);
	if (e.key.toLowerCase() !== n.toLowerCase()) return !1;
	let i = r.includes("ctrl"), a = r.includes("shift"), o = r.includes("alt"), s = r.includes("meta");
	return !(i !== e.ctrlKey || a !== e.shiftKey || o !== e.altKey || s !== e.metaKey);
}
var EventHandlers = class {
	...
	onKeyDown(e) {
		if (!this.handleTransformModeKeys(e)) {
			for (let [t, n] of Object.entries(this.keybindConfig)) if (n.some((t) => matchKey$1(e, t))) {
				let e = this.actionMap[t];
				e && e();
				return;
			}
		}
	...
	rotateSelectedObjects(e, t, n = null) {
		let r = e;
		e && typeof e == "object" && Object.prototype.hasOwnProperty.call(e, "rotationAxis") && ({rotationAxis: r, rotationAngle: t, selectedObjects: n = null} = e), n === null && (n = this.selectionManager.selectedObjects), r = r.normalize(), t = THREE.MathUtils.degToRad(t), n.forEach((e) => {
			e.rotateOnAxis(r, -t);
		});
	}
	...
	copySelectedObjects() {
		...
		return this.selectionManager.selectedObjects.forEach((t) => {
			let n = t.clone();
			n.position.add(new THREE.Vector3(1, 1, 1)), this.scene.add(n), e.push(n);
...
function initVertexIndicators(e) {
	let t = e.geometry.attributes.position, n = t.count, r = new THREE.PointsMaterial({
		vertexColors: !0,
		size: 5,
		sizeAttenuation: !1
	...
	i.setAttribute("color", new THREE.BufferAttribute(a, 3));
	let o = new THREE.Points(i, r);
...
var _frustum = new Frustum(), _center = new Vector3(), _tmpPoint = new Vector3(), _vecNear = new Vector3(), _vecTopLeft = new Vector3(), _vecTopRight = new Vector3(), _vecDownRight = new Vector3(), _vecDownLeft = new Vector3(), _vecFarTopLeft = new Vector3(), _vecFarTopRight = new Vector3(), _vecFarDownRight = new Vector3(), _vecFarDownLeft = new Vector3(), _vectemp1 = new Vector3(), _vectemp2 = new Vector3(), _vectemp3 = new Vector3(), _matrix = new Matrix4(), _quaternion = new Quaternion(), _scale = new Vector3(), SelectionBox = class {
	...
	highlightSelectedVertex() {
		Object.keys(this.selectedInstances).forEach((e) => {
			...
			}), n.geometry.setAttribute("color", new THREE.BufferAttribute(a, 3)), n.geometry.attributes.color.needsUpdate = !0;
		});
	}
	...
	updateAxisLine() {
		...
		if (!e[t] || !e[n]) return;
		let r = new THREE.Vector3(...e[t]), i = new THREE.Vector3(...e[n]), a = i.clone().sub(r), o = a.length();
		if (o === 0) return;
		...
		if (this.axisLine) this.axisLine.geometry.setFromPoints([u, d]), this.axisLine.geometry.attributes.position.needsUpdate = !0, this.axisLine.geometry.computeBoundingSphere();
			...
			let e = new THREE.BufferGeometry().setFromPoints([u, d]), t = new THREE.LineBasicMaterial({
				color: 16746496,
				transparent: !0,
				opacity: .9,
				depthTest: !1
			});
			this.axisLine = new THREE.Line(e, t), this.axisLine.userData.notSelectable = !0, this.axisLine.layers.set(1), this.axisLine.renderOrder = 999, this.tjs.scene.add(this.axisLine);
		}
	}
	refreshAxisLine() {
		this.updateAxisLine();
	...
	showRotatePlaneFromAxisAtoms() {
		...
		let i = new THREE.Vector3(...e[t]), a = new THREE.Vector3(...e[n]), o = new THREE.Vector3(...e[r]), s = a.clone().sub(i).cross(o.clone().sub(i));
		if (s.lengthSq() === 0) {
			this.hidePlaneWithNormal({
				meshKey: "rotatePlaneMesh",
				lineKey: "rotateNormalLine"
	...
	showPlaneWithNormal({ center: e, normal: t, size: n, color: r, lineLength: i, meshKey: a, lineKey: o }) {
		...
		if (!this[a]) {
			let e = new THREE.PlaneGeometry(n, n), t = new THREE.MeshBasicMaterial({
				color: r,
				transparent: !0,
				opacity: .15,
			...
			this[a] = new THREE.Mesh(e, t), this[a].userData.notSelectable = !0, this[a].layers.set(1), this[a].renderOrder = 998, this.tjs.scene.add(this[a]);
		}
		let c = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 0, 1), s);
		...
		if (this[o]) this[o].geometry.setFromPoints([l, u]), this[o].geometry.attributes.position.needsUpdate = !0, this[o].geometry.computeBoundingSphere(), this[o].computeLineDistances();
			...
			let e = new THREE.BufferGeometry().setFromPoints([l, u]), t = new THREE.LineDashedMaterial({
				color: r,
				dashSize: .6,
				gapSize: .4,
				transparent: !0,
			...
			this[o] = new THREE.Line(e, t), this[o].computeLineDistances(), this[o].userData.notSelectable = !0, this[o].layers.set(1), this[o].renderOrder = 999, this.tjs.scene.add(this[o]);
		}
	...
	showAxisLine({ center: e, axis: t, length: n, color: r, lineKey: i }) {
		...
		if (this[i]) this[i].geometry.setFromPoints([o, s]), this[i].geometry.attributes.position.needsUpdate = !0, this[i].geometry.computeBoundingSphere(), this[i].computeLineDistances();
			...
			let e = new THREE.BufferGeometry().setFromPoints([o, s]), t = new THREE.LineDashedMaterial({
				color: r,
				dashSize: .6,
				gapSize: .4,
				transparent: !0,
			...
			this[i] = new THREE.Line(e, t), this[i].computeLineDistances(), this[i].userData.notSelectable = !0, this[i].layers.set(1), this[i].renderOrder = 999, this.tjs.scene.add(this[i]);
		}
		this.weas?.avr?.requestRedraw?.("render");
	}
	...
	getAtomsInsideLasso(e) {
		...
		let n = this.tjs.updateViewerRect(), r = this.tjs.camera, i = [], a = new THREE.Vector3();
...
function getClosestVertex(e, t, n) {
	let r = e.geometry.getAttribute("position"), i = new THREE.Vector3(), a = new THREE.Vector3(), o = new THREE.Vector3();
...
//#endregion
//#region src/operation/OperationSearch.js
...
var OperationSearchManager = class {
	constructor(e, t) {
	...
	createOverlay() {
		...
		t.type = "text", t.id = "search-box", t.placeholder = "Search operation...", t.addEventListener("input", (e) => this.updateSearchResults(e.target.value));
		let n = document.createElement("ul");
		return n.id = "search-results", e.appendChild(t), e.appendChild(n), this.weas.tjs.containerElement.appendChild(e), e;
	}
	bindEvents() {
		...
		[
			...
			if ((this.keybindConfig.SearchOperation || []).some((t) => matchKey(e, t))) {
...
//#endregion
//#region src/geometry/utils.js
function pointsInsideMesh(e, t) {
	let n = new THREE.Raycaster(), r = new THREE.Vector3(.23184, .413, .879), i, a = [];
	for (let o = 0; o < e.length; o++) i = new THREE.Vector3(...e[o]), n.set(i, r), n.intersectObject(t).length % 2 == 1 && a.push(o);
	return a;
}
//#endregion
...
var StateStore = class {
	...
	drawMesh() {
		this.clearMeshes(), this.settings.forEach((e) => {
			...
			let i = new THREE.InstancedMesh(t, r, e.instances.length);
			r.opacity < 1 && (i.renderOrder = 2), e.instances.forEach((e, t) => {
				let n = new THREE.Object3D(), r = new THREE.Vector3(...e.position);
				n.position.copy(r);
				let a = e.scale || [
					1,
					1,
				...
				i.setColorAt(t, new THREE.Color(s));
		...
		switch (e.type) {
			case "cube":
				n = {
					...
					...n,
					...e.shape
				}, t = new THREE.BoxGeometry(r.width, r.height, r.depth);
				break;
			case "cylinder":
				n = {
					radiusTop: 1,
					...
					...n,
					...e.shape
				}, t = new THREE.CylinderGeometry(r.radiusTop, r.radiusBottom, r.height, r.radialSegments, r.heightSegments, r.openEnded);
				break;
			case "icosahedron":
				n = {
					radius: 1,
					...
					...n,
					...e.shape
				}, t = new THREE.IcosahedronGeometry(r.radius, r.detail);
				break;
			case "cone":
				n = {
					radius: 1,
					...
					...n,
					...e.shape
				}, t = new THREE.ConeGeometry(r.radius, r.height, r.radialSegments, r.heightSegments, r.openEnded);
				break;
			case "plane":
				n = {
					width: 1,
					...
					...n,
					...e.shape
				}, t = new THREE.PlaneGeometry(r.width, r.height);
				break;
			case "sphere":
				n = {
					radius: 1,
					...
					...n,
					...e.shape
				}, t = new THREE.SphereGeometry(r.radius, r.widthSegments, r.heightSegments, r.phiStart, r.phiLength, r.thetaStart, r.thetaLength);
				break;
			case "torus":
				n = {
					radius: 1,
					...
					...n,
					...e.shape
				}, t = new THREE.TorusGeometry(r.radius, r.tube, r.radialSegments, r.tubularSegments, r.arc);
				break;
...
var Setting$7 = class {
	...
	drawMesh() {
		this.clearMeshes(), this.settings.forEach((e) => {
			...
			Array.isArray(e.color) ? n.color.setRGB(e.color[0], e.color[1], e.color[2]) : n.color = new THREE.Color(e.color);
			let r = e.opacity ?? 1;
			n.transparent = !0, n.opacity = r, n.side = {
				FrontSide: THREE.FrontSide,
				BackSide: THREE.BackSide,
				DoubleSide: THREE.DoubleSide
			}[e.side] ?? THREE.DoubleSide, n.depthWrite = e.depthWrite ?? !0, n.depthTest = e.depthTest ?? !0;
			let i = new THREE.BufferGeometry(), a = new Float32Array(e.vertices);
			i.setAttribute("position", new THREE.BufferAttribute(a, 3));
			let o = new Uint32Array(e.faces);
			i.setIndex(new THREE.BufferAttribute(o, 1));
			let s = i;
			s = mergeVertices(i, e.mergeVerticesTolerance), e.mergeVerticesTolerance, (e.smoothNormals ?? !0) && s.computeVertexNormals();
			let c = new THREE.Mesh(s, n), l = e.selectable ?? !0, u = typeof e.layer == "number" ? e.layer : l ? 0 : 1;
			if (c.userData.anyMeshName = e.name, c.userData.type = "anyMesh", c.userData.uuid = this.viewer.uuid, c.userData.notSelectable = !l, e.userData && typeof e.userData == "object" && Object.assign(c.userData, e.userData), c.layers.set(u), c.visible = e.visible ?? !0, c.position.set(e.position[0], e.position[1], e.position[2]), typeof e.renderOrder == "number" && (c.renderOrder = e.renderOrder), this.meshes.push(c), this.scene.add(c), e.showEdges) {
				let t = e.edgeColor || [
					...
					0,
					1
				], n = t.length === 4 ? t[3] : 1, r = new THREE.LineBasicMaterial({
					color: new THREE.Color(t[0], t[1], t[2]),
					transparent: n < 1,
					opacity: n
				}), i = new THREE.EdgesGeometry(s), a = new THREE.LineSegments(i, r);
...
var DEFAULT_TEXT_SETTINGS = {
	...
	constructor(e, { sceneName: t = "MainScene" } = {}) {
	...
	addLabel(e = {}) {
		let { text: t, position: n, color: r, fontSize: i, className: a, renderMode: o, clampFont: s = !0 } = {
			...
			...e
		}, c = createLabel(new THREE.Vector3(...n), t, r, normalizeFontSize(i, s), a);
...
var require_lodash_merge = /* @__PURE__ */ __commonJSMin(((e, t) => {
	...
	drawUnitCellVectors() {
		let e = new THREE.Vector3(0, 0, 0), t = this.viewer.originalCell, n = 1.5;
		if (!t || t.length !== 3) {
			console.warn("Invalid or missing unit cell data for vectors");
			return;
		}
		if (!this.viewer.tjs.hud.miniScenes.get("coord")) return;
		let r = this.viewer.tjs.hud.coordAxesGroup, i = new THREE.Group(), a = [
			"a",
			"b",
			"c"
		], o = this.settings.hudAxisColors, s = .5;
		return t.forEach((t, r) => {
			let s = new THREE.Vector3(...t).clone().normalize().clone().multiplyScalar(n), c = this.shapeRegistry.create("Arrow", {
				color: o[a[r]],
				start: e.clone(),
				end: s.clone(),
				shaftRadius: .06,
...
function createSpriteLabel(e, t, n, r) {
	...
	let s = new THREE.CanvasTexture(a);
	s.minFilter = THREE.LinearFilter, s.generateMipmaps = !1, s.encoding = THREE.sRGBEncoding, s.anisotropy = 16;
	let c = new THREE.Sprite(new THREE.SpriteMaterial({ map: s }));
	c.position.copy(e);
	let l = 2.25;
...
function getAtomColors(e, t, n) {
	...
	if (t === "Random") r = [], e.symbols.forEach((e, t) => {
		i = new THREE.Color(Math.random() * 16777215), r.push(i);
	});
	else if (t === "Uniform") r = [], e.symbols.forEach((e, t) => {
		i = new THREE.Color(n.colorRamp[0]), r.push(i);
	});
...
function getColorsFromArray(e, t) {
	...
	return e.forEach((e) => {
		let o = (e - r) / i, s = Math.min(Math.floor(o * a), a - 1), c = (o - s / a) * a, l = new THREE.Color(t[s]), u = new THREE.Color(t[s + 1]), d = new THREE.Color(l.r, l.g, l.b).lerp(u, c);
		n.push(d);
	}), n;
}
//#endregion
...
function convertColor$1(e) {
	return e = Array.isArray(e) ? new THREE.Color(...e) : new THREE.Color(e), e;
}
function drawAtoms({ atoms: e, atomScales: t, settings: n, colors: r, materialType: i = "Standard", shapeType: a = "Sphere", data_type: o = "atom", shapeRegistry: s }) {
	...
	if (l instanceof THREE.Mesh) u = l.geometry.clone(), d = l.material.clone();
	else if (l instanceof THREE.Group) {
		let e = l.children.find((e) => e instanceof THREE.Mesh);
	...
	let f = e.symbols.length, p = new THREE.InstancedMesh(u, d, f);
	p.instanceColor = new THREE.InstancedBufferAttribute(new Float32Array(f * 3), 3);
	let m = new THREE.Vector3(), h = new THREE.Quaternion(), g = new THREE.Vector3(), _ = new THREE.Matrix4();
	return e.symbols.forEach((i, a) => {
		m.set(...e.positions[a]);
		let o = (i in n ? n[i].radius : 1) * t[a];
		g.set(o, o, o), _.compose(m, h, g), p.setMatrixAt(a, _);
		let s = r[a] instanceof THREE.Color ? r[a] : new THREE.Color(r[a]);
		p.setColorAt(a, s);
...
var Setting$5 = class {
	...
	updateAtomColors() {
		...
		this.viewer.atoms.symbols.forEach((t, n) => {
			...
			let r = new THREE.Color(this.settings[t].color);
			e.push(r);
	...
	drawBalls() {
		...
		if (this.viewer.imageAtomsList = this.viewer.bondedAtoms.atoms.concat(t), this.imageAtomMap = createImageAtomsMapping(this.viewer.imageAtomsList), this.viewer.imageAtomsList.length > 0) {
			...
			let r = this.viewer.imageAtomsList.map(([e]) => {
				...
				if (n) return typeof n.clone == "function" ? n.clone() : new THREE.Color(n);
				let r = this.viewer.atoms.symbols[e];
				return this.settings[r] || (this.settings[r] = this.getDefaultSetting(r, t.species[r]?.element || r)), new THREE.Color(this.settings[r].color);
			}), i = drawAtoms({
				scene: this.scene,
				atoms: t,
				atomScales: n,
	...
	updateAtomMesh(e = null, t = null) {
		...
		for (let e = 0; e < t.positions.length; e++) this.meshes.atom.getMatrixAt(e, n), n.setPosition(new THREE.Vector3(...t.positions[e])), this.meshes.atom.setMatrixAt(e, n), this.updateImageAtomsMesh(e);
	...
	updateImageAtomsMesh(e) {
		this.viewer.imageAtomsList.length > 0 && this.imageAtomMap[e] && this.imageAtomMap[e].forEach((t) => {
			let n = t.index, r = this.viewer.atoms.positions[e].map((e, n) => e + calculateCartesianCoordinates(this.viewer.atoms.cell, t.offset)[n]), i = new THREE.Matrix4();
			this.meshes.image.getMatrixAt(n, i), i.setPosition(new THREE.Vector3(...r)), this.meshes.image.setMatrixAt(n, i);
		});
	}
	updateAtomScale(e) {
	...
	updateMeshScale(e, t, n, r) {
		let i = new THREE.Vector3(), a = new THREE.Quaternion(), o = new THREE.Vector3();
		t.forEach((t) => {
			let s = new THREE.Matrix4(), c = this.settings[n[t]].radius || 1;
...
var Node = class {
	...
	drawBonds() {
		...
		let c = drawLine(this.viewer.originalAtoms, this.bondList, this.bondMap.stickCaps, this.viewer.cutoffs, this.bondRadius, this.viewer._materialType, r);
		return this.meshes = {
			stickBondMesh: i,
			stickCapBondMesh: a,
			stickCapBondCap: o,
	...
	updateBondStick(e = null, t = null, n, r = null, i = "sticks") {
		...
		a.forEach((e) => {
			...
			u = new THREE.Vector3(...u), d = new THREE.Vector3(...d);
			let f = new THREE.Vector3().lerpVectors(u, d, .25), p = t.symbols[o] + "-" + t.symbols[s];
			if (!this.viewer.cutoffs[p]) return;
			let m = this.viewer.cutoffs[p].max, h = calculateQuaternion(u, d), g = calculateScale(u, d, this.bondRadius, m, this.hideLongBonds), _ = new THREE.Matrix4().compose(f, h, g);
			n.setMatrixAt(e * 2, _);
			let v = new THREE.Vector3().lerpVectors(u, d, .75), y = new THREE.Matrix4().compose(v, h, g);
			if (n.setMatrixAt(e * 2 + 1, y), r) {
				let t = new THREE.Vector3(this.bondRadius, this.bondRadius, this.bondRadius), n = new THREE.Matrix4().compose(u, new THREE.Quaternion(), t);
				r.setMatrixAt(e * 2, n);
				let i = new THREE.Matrix4().compose(d, new THREE.Quaternion(), t);
	...
	updateBondLine(e = null, t = null, n, r = "dashedLines") {
		...
		i.forEach((e) => {
			...
			l = new THREE.Vector3(...l), u = new THREE.Vector3(...u);
...
function drawStick({ atoms: e, bondList: t, bondIndices: n, settings: r, radius: i = .1, materialType: a = "Standard", atomColors: o = null, withCap: s = !1, logger: c = console, shapeRegistry: l }) {
	...
	if (d instanceof THREE.Mesh) p = d.geometry.clone(), d.material.clone();
	else throw Error("Cylinder shape must be a mesh");
	let m;
	if (f) if (f instanceof THREE.Mesh) m = f.geometry.clone();
	else throw Error("Sphere shape must be a mesh");
	let h = d.material.clone();
	h.color.set(16777215), h.transparent = !0;
	let g = performance.now(), _ = new THREE.InstancedMesh(p, h, n.length * 2), v = s ? new THREE.InstancedMesh(m, h, n.length * 2) : null, y = new THREE.Vector3(), b = new THREE.Vector3(), x = new THREE.Vector3(), w = new THREE.Vector3(), T = new THREE.Matrix4();
	for (let a = 0; a < n.length; a++) {
		...
		if (T.compose(x, E, k), _.setMatrixAt(a * 2, T), _.setColorAt(a * 2, h), T.compose(w, E, k), _.setMatrixAt(a * 2 + 1, T), _.setColorAt(a * 2 + 1, g), s) {
			let e = new THREE.Vector3(i, i, i), t = new THREE.Matrix4().compose(y, new THREE.Quaternion(), e);
			v.setMatrixAt(a * 2, t), v.setColorAt(a * 2, h);
			let n = new THREE.Matrix4().compose(b, new THREE.Quaternion(), e);
...
function drawLine(e, t, n, r, i = "dashed", a = null) {
	...
	n.forEach((n) => {
		...
		d = new THREE.Vector3(...d), o.push(u.x, u.y, u.z), o.push(d.x, d.y, d.z);
	...
	c.setAttribute("position", new THREE.Float32BufferAttribute(o, 3)), c.setAttribute("color", new THREE.Float32BufferAttribute(s, 3));
	let l;
	l = i === "dashed" ? new THREE.LineDashedMaterial({
		color: 16777215,
		vertexColors: !0,
		dashSize: .1,
		gapSize: .1,
	...
	let u = new THREE.LineSegments(c, l);
...
function calculateScale(e, t, n, r = null, i = !0) {
	...
	return i && r !== null && a > r && (n = 0), new THREE.Vector3(n, a / 2, n);
}
...
var Visible = 0, Deleted = 1, _v1 = new Vector3(), _line3 = new Line3(), _plane = new Plane(), _closestPoint = new Vector3(), _triangle = new Triangle(), ConvexHull = class {
	...
	buildPolyhedras(e, t, n, r = "CPK", i = "standard") {
		...
		for (let i of Object.keys(t)) {
			...
			try {
				...
				a.push(...n), o.push(...i);
				let p = e.symbols[d.atomIndex], m = elementColors[r][p] || defaultColor, h = new THREE.Color(m);
	...
	drawPolyhedraMesh(e, t = "standard") {
		...
		n.transparent = !0, n.opacity = .5, n.vertexColors = !0, n.side = THREE.DoubleSide, n.depthWrite = !1, n.depthTest = !0;
		let r = new THREE.BufferGeometry();
		r.setAttribute("position", new THREE.Float32BufferAttribute(this.allVertices, 3)), r.setAttribute("normal", new THREE.Float32BufferAttribute(this.allNormals, 3)), r.setAttribute("color", new THREE.Float32BufferAttribute(this.allColors, 3));
		let i = new THREE.Mesh(r, n);
...
var DEFAULT_LABEL_SETTING = {
	...
	applySettings(e, t = []) {
		this.settings = [], this.overlaySettings = [], clearLabels(this.scene, this.labels), e.forEach((e) => {
			this.addSetting(e);
		}), t.forEach((e) => {
	...
	getAtomRadiusScale(e) {
		...
		let n = new THREE.Matrix4(), r = new THREE.Vector3(), i = new THREE.Quaternion(), a = new THREE.Vector3();
		return t.getMatrixAt(e, n), n.decompose(r, i, a), a.x || a.y || a.z || null;
	}
};
function clearLabels(e, t) {
	t.forEach((t) => {
		e.remove(t), t.remove();
	});
}
//#endregion
//#region src/geometry/marchingCubes.js
...
function normalizeHexColor(e) {
	return "#" + (e instanceof THREE.Color ? e : new THREE.Color(e)).getHexString();
}
var Setting$2 = class {
...
var FermiSurface = class {
	...
	drawSlices() {
		...
		let e = this.viewer.volumetricData, t = e.values, n = e.dims, r = e.cell, i = new THREE.Vector3(...e.origin);
		Object.entries(this.settings).forEach(([e, a]) => {
			...
			if (a.method === "miller") o = computePlaneNormalFromMillerIndices(a.h, a.k, a.l, r), s = o.clone().multiplyScalar(a.distance).add(i);
				...
				let e = computeBestFitPlane(a.selectedAtomIndices.map((e) => new THREE.Vector3(...this.viewer.atoms.positions[e])));
...
function computePlaneNormalFromMillerIndices(e, t, n, r) {
	let i = new THREE.Vector3(...r[0]), a = new THREE.Vector3(...r[1]), o = new THREE.Vector3(...r[2]), s = i.dot(a.clone().cross(o)), c = a.clone().cross(o).multiplyScalar(2 * Math.PI / s), l = o.clone().cross(i).multiplyScalar(2 * Math.PI / s), u = i.clone().cross(a).multiplyScalar(2 * Math.PI / s);
...
function computeBestFitPlane(e) {
	...
	return {
		normal: new THREE.Vector3(...u).normalize(),
		point: t
	};
}
function extractSliceArbitrary(e, t, n, r, i, a, o) {
	...
	for (let i = 0; i < v; i++) {
		let s = [];
		for (let f = 0; f < _; f++) {
			let p = u + f * o, m = d + i * o, h = new THREE.Vector2(p, m), g = 0;
...
function cellToGridCoordinates(e, t, n) {
	let r = new THREE.Vector3(...t[0]), i = new THREE.Vector3(...t[1]), a = new THREE.Vector3(...t[2]), o = new THREE.Matrix3();
...
function createTextureFromSlice(e, t, n, r) {
	...
	let l = new THREE.DataTexture(o, i, a, THREE.RGBAFormat);
	return l.needsUpdate = !0, l.minFilter = THREE.LinearFilter, l.magFilter = THREE.LinearFilter, l;
}
function createSlicePlaneArbitrary(e, t, n, r, i, a, o, s, c) {
	let l = computePlaneBasis(t), u = new THREE.Shape(e), d = new THREE.ShapeGeometry(u);
	...
	let f = new THREE.Matrix4(), p = l.u, m = l.v, h = t;
...
function getUnitCellVertices(e, t) {
	let n = new THREE.Vector3(...e[0]), r = new THREE.Vector3(...e[1]), i = new THREE.Vector3(...e[2]);
	return [
		t.clone(),
		t.clone().add(n),
		t.clone().add(r),
	...
	updateArrowMesh(e = null, t = null) {
		t === null && (t = this.viewer.atoms), Object.entries(this.settings).forEach(([t, n]) => {
			...
			(e === null ? [...Array(o).keys()] : [e]).forEach((e) => {
				let t = new THREE.Vector3(...r[e]), o = new THREE.Vector3(...i[e]).multiplyScalar(n.factor), s = t.clone().add(o), c = new THREE.Vector3().lerpVectors(t, s, 0), l = calculateQuaternion(t, s), u = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), Math.PI);
				l.multiply(u);
				let d = new THREE.Vector3(10 * n.radius, t.distanceTo(s), 10 * n.radius), f = new THREE.Matrix4().compose(c, l, d);
				a.setMatrixAt(e, f);
...
function drawAtomArrows({ length: e = 0, color: t = 0, materialType: n = "Standard", shapeRegistry: r }) {
	...
	if (!(i instanceof THREE.Mesh)) throw Error("Arrow must return a THREE.Mesh");
	let a = i.geometry.clone(), o = i.material.clone();
	o.color.set(t);
	let s = new THREE.InstancedMesh(a, o, e);
...
var DEFAULT_MEASUREMENT_SETTINGS = {
	...
	drawHighlightAtoms() {
		...
		let e = new THREE.MeshBasicMaterial({
			color: "yellow",
			opacity: .6,
			transparent: !0
		});
		e.depthWrite = !1, e.depthTest = !0;
		let t = new THREE.SphereGeometry(1, 16, 16);
		this.drawHighlightMesh("sphere", t, e);
		let n = e.clone();
		n.color = "green";
		let r = new THREE.BoxGeometry(2, 2, 2);
		this.drawHighlightMesh("box", r, n);
		let i = new THREE.MeshBasicMaterial({
			color: 16777215,
			opacity: 1,
			transparent: !0,
			vertexColors: !0
		...
		let o = new THREE.MeshBasicMaterial({
			color: 16777215,
			opacity: 1,
			transparent: !0,
			side: THREE.DoubleSide,
	...
	drawHighlightMesh(e, t, n) {
		...
		if (!r) return;
		let i = new THREE.InstancedMesh(t, n, r.count), a = new THREE.Vector3(), o = new THREE.Quaternion(), s = new THREE.Vector3(), c = new THREE.Matrix4(), l = new THREE.Matrix4();
	...
	createCrossGeometry(e = 1) {
		let t = new THREE.TorusGeometry(e, .1, 16, 20), n = new THREE.TorusGeometry(e, .1, 16, 20);
		n.rotateX(Math.PI / 2);
		let r = new THREE.TorusGeometry(e, .1, 16, 20);
	...
	updateHighlightAtomsMesh({ indices: e = [], scale: t = 1.1, color: n = "yellow", type: r = "sphere", opacity: i = null, occlude: a = !0, offset: o = 1.0005, thickness: s = null }, c = null) {
		...
		if (this.viewer.atoms.symbols.length > 0 && this.meshes[r]) {
			...
			let a = new THREE.Vector3(), o = new THREE.Quaternion(), s = new THREE.Vector3();
			e.forEach((e) => {
	...
	addTimeline() {
		if (this.div.querySelector("#animation-controls")) return;
		let e = document.createElement("div");
		...
		let n = 100;
		this.timeline.max = 100, this.playPauseBtn.addEventListener("click", () => {
	...
	removeTimeline() {
		let e = this.div.querySelector("#animation-controls");
		e && e.remove();
	}
	...
	addLegend() {
		this.removeLegend();
		let e = document.createElement("div");
		e.id = "legend-container", e.style.position = "absolute", e.style.backgroundColor = "rgba(255, 255, 255, 0.8)", e.style.padding = "10px", e.style.borderRadius = "5px", e.style.zIndex = "1000", this.setLegendPosition(e), Object.entries(this.viewer.atomManager.settings).forEach(([t, n]) => {
			let r = document.createElement("div");
			r.style.display = "flex", r.style.alignItems = "center", r.style.marginBottom = "5px";
			let i = document.createElement("canvas");
			i.width = 20, i.height = 20;
			let a = i.getContext("2d");
			a.fillStyle = `#${n.color.getHexString()}`;
			let o = n.radius * 10;
			a.beginPath(), a.arc(10, 10, o, 0, Math.PI * 2), a.fill(), r.appendChild(i);
			let s = document.createElement("span");
			s.textContent = ` ${t}`, s.style.marginLeft = "5px", s.style.fontSize = "14px", r.appendChild(s), e.appendChild(r);
		}), this.viewer.tjs.containerElement.appendChild(e);
	}
	removeLegend() {
		let e = this.viewer.tjs.containerElement.querySelector("#legend-container");
		e && e.remove();
	}
	updateLegend() {
	...
	setLegendPosition(e) {
		let t = this.getAtomLegendConfig().position || "top-right";
...
var Phonon = class {
	...
	updateAtoms(e) {
		...
		try {
			...
			if (this.baseAtomLabelSettings = this.getAtomLabelSettingsFromType(this._atomLabelType), this.updateAtomLabels(), this.weas.textManager?.redraw?.(), this.drawModels(), (this._autoResetCameraOnAtomsUpdate || !this._hasInitializedCamera) && this.atoms && this.atoms.getAtomsCount() > 0) {
				...
				let e = this.atoms.getCenterOfGeometry();
		...
		try {
			n || this.state.set({ viewer: e }), Object.entries(e).forEach(([e, t]) => {
				...
				if (this[e] = n, e === "radiusType" && (this.atomManager.init(), this.bondManager.init(), this.polyhedraManager.init()), e === "colorBy" && (this.atomManager.init(), this.bondManager.init(), this.polyhedraManager.init()), e === "colorType" && (this.atomManager.init(), this.guiManager.updateLegend(), this.bondManager.init(), this.polyhedraManager.init()), e === "atomScale" && (this.atomManager.updateAtomScale(t), Object.values(this.highlightManager.settings || {}).forEach((e) => {
					this.highlightManager.updateHighlightAtomsMesh(e);
				})), e === "backgroundColor" && (this.tjs.scene.background = new THREE.Color(t)), e === "atomLabelType" && (this.baseAtomLabelSettings = this.getAtomLabelSettingsFromType(t), this.updateAtomLabels()), e === "modelStyle" && a && this.updateModelStyles(n), r) {
					let t = this.getRedrawEffectForKey(e);
					t && this.requestRedraw(t);
				}
	...
	drawModels() {
		...
		this.atomManager.meshes.atom.add(n), this.isosurfaceManager.drawIsosurfaces(), this.volumeSliceManager.drawSlices(), this.VFManager.drawVectorFields(), this.highlightManager.drawHighlightAtoms(), this.ALManager.drawAtomLabels(), this.guiManager.updateLegend(), this.ready = !0, this.requestRedraw("render");
	}
	dispose() {
	...
	setAtomPosition({ index: e, position: t }) {
		let n = this.wrapPositionIfNeeded(t), r = new THREE.Matrix4();
	...
	translateSelectedAtoms({ translateVector: e, indices: t = null }) {
		t === null && (t = this.selectedAtomsIndices), t = toIndexArray(t), t.length !== 0 && (e = toVector3(e, "translateVector"), t.forEach((t) => {
			let n = new THREE.Vector3(...this.atoms.positions[t]).clone().add(e);
			this.setAtomPosition({
				index: t,
				position: n
			});
	...
	rotateSelectedAtoms({ cameraDirection: e, rotationAngle: t, indices: n = null, centroid: r = null }) {
		...
		let i = new THREE.Matrix4().makeRotationAxis(e, -t);
		n === null && (n = this.selectedAtomsIndices), n = toIndexArray(n), n.length !== 0 && (r === null && (r = new THREE.Vector3(0, 0, 0), n.forEach((e) => {
			r.add(new THREE.Vector3(...this.atoms.positions[e]));
		}), r.divideScalar(n.length)), r = toVector3(r, "centroid"), n.forEach((e) => {
			let t = new THREE.Vector3(...this.atoms.positions[e]);
...
var MaterialsRegistry = class {
	...
	_createBaseMesh(e, t, { materialType: n = "Standard", color: r = "#bd0d87", opacity: i = 1, wireframe: a = !1 } = {}) {
		...
		return "color" in o && (o.color = new THREE.Color(r)), o.opacity = i, o.transparent = i < 1, o.depthWrite = i >= 1, o.side = i < 1 ? THREE.DoubleSide : THREE.FrontSide, o.wireframe = a, new THREE.Mesh(t, o);
	}
	_registerBuiltIns() {
		let e = 1;
		this.register("Cube", (e, t) => this._createBaseMesh(e, new THREE.BoxGeometry(2, 2, 2), t)), this.register("Sphere", (e, t) => {
			let n = t.widthSegments ?? 32, r = t.heightSegments ?? 32;
			return this._createBaseMesh(e, new THREE.SphereGeometry(1, n, r), t);
		}), this.register("Plane", (e, t) => this._createBaseMesh(e, new THREE.PlaneGeometry(2, 2), t)), this.register("Cylinder", (e, t) => {
			let n = t.segments ?? 24;
			return this._createBaseMesh(e, new THREE.CylinderGeometry(1, 1, 1, n), t);
		}), this.register("Cone", (e, t) => this._createBaseMesh(e, new THREE.ConeGeometry(1, 2, 16), t)), this.register("Torus", (e, t) => this._createBaseMesh(e, new THREE.TorusGeometry(1 * .75, 1 * .25, 16, 32), t)), this.register("Line", (e, t) => {
			let n = t.color || "#000000", r = t.start || [
				...
				1,
				0
			], a = [new THREE.Vector3(...r), new THREE.Vector3(...i)], o = new THREE.BufferGeometry().setFromPoints(a), s = new THREE.LineBasicMaterial({ color: n });
			return new THREE.Line(o, s);
		}), this.register("Arrow", (e, t) => {
			let n = t.shaftRatio ?? .75, r = t.length ?? 1, i = r * n, a = r - i, o = (t.shaftRadius ?? .075) * r, s = (t.headRadius ?? .15) * r, c = new THREE.CylinderGeometry(o, o, i, t.segments ?? 12);
			c.translate(0, i / 2, 0);
			let l = new THREE.ConeGeometry(s, a, t.segments ?? 12);
			l.translate(0, i + a / 2, 0);
			let u = mergeGeometries([c, l], !1), d = this._createBaseMesh(e, u, t);
			if (t.start && t.end) {
				let e = new THREE.Vector3(...t.start), n = new THREE.Vector3(...t.end), i = new THREE.Vector3().subVectors(n, e), a = i.length();
				i.normalize(), d.scale.set(1, a / r, 1);
				let o = new THREE.Vector3(0, 1, 0).cross(i), s = Math.acos(new THREE.Vector3(0, 1, 0).dot(i));
			...
			let n = new ConvexGeometry(t.corners.map((e) => e.isVector3 ? e : new THREE.Vector3(...e)));
			if (t.edges) {
				let e = new THREE.EdgesGeometry(n), r = new THREE.LineBasicMaterial({ color: t.color ?? 16777215 }), i = new THREE.LineSegments(e, r);
				return i.updateCorners = (e) => {
					let t = e.map((e) => e.isVector3 ? e : new THREE.Vector3(...e));
					i.geometry.dispose(), i.geometry = new THREE.EdgesGeometry(new ConvexGeometry(t));
				}, i;
			}
	...
	create(e, t = {}) {
		...
		if (r instanceof THREE.Object3D && (t.position && r.position.set(...t.position), t.scale && r.scale.set(...t.scale), t.rotation)) {
			let [e, n, i] = t.rotation;
			r.rotation.set(THREE.MathUtils.degToRad(e), THREE.MathUtils.degToRad(n), THREE.MathUtils.degToRad(i));
		}
	...
	constructor({ domElement: e, atoms: t = [new Atoms()], viewerConfig: n = {}, guiConfig: r = {}, tjsConfig: i = null, keybindConfig: a = null }) {
		this.uuid = THREE.MathUtils.generateUUID(), this.tjsConfig = i, this.tjs = new BlendJS(e, this), this.keybindConfig = a, this.materialsRegistry = new MaterialsRegistry(), this.shapeRegistry = new ShapeRegistry(this.materialsRegistry), this.tjs.requestRedraw = this.requestRedraw.bind(this), this.guiManager = new GUIManager(this, r), this.eventHandlers = new EventHandlers(this), this.ops = new OperationManager(this), this.selectionManager = new SelectionManager(this), this.objectManager = new ObjectManager(this), this.state = new StateStore(createDefaultState()), this.textManager = new TextManager(this.tjs.scene), this.avr = new AtomsViewer({
			weas: this,
			atoms: t,
			viewerConfig: n
	...
	exportState() {
		...
		}), t.plugins && (this.anyMesh && (t.plugins.anyMesh = { settings: cloneValue(this.anyMesh.settings || []) }), this.instancedMeshPrimitive && (t.plugins.instancedMeshPrimitive = { settings: cloneValue(this.instancedMeshPrimitive.settings || []) })), {
			version: "weas_state_v1",
			atoms: e,
			state: t,
